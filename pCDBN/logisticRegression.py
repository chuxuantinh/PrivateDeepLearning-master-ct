...
class dpLogisticRegression(object):
    ...
    def cost(self, y):
        ...
        Since '* z' is an element-wise multiplication and 'z' is one_hot encoding, h can be considered coefficients of the term z * h. By applying Funtional Mechanism, we perturb (z * h) * W as tf.matmul(h + LaplaceNoise, W) * z:
                
        h += LaplaceNoise; where
                
        LaplaceNoise = np.random.laplace(0.0, scale3, hk)
        ...
        where scale3 = 10*(hk + 1/4 * hk**2)/(epsilon3*batch_size); (Lemma 5)
