...
class EncLayer(object):
    ...
    def propup(self, v, W, b, bn_index):
      ...
      h = tf.layers.batch_normalization(h, scale=True, training=True, 
                                        reuse=tf.AUTO_REUSE, name='aebn{}'.format(bn_index))
      return tf.clip_by_value(h, -1, 1) # values of hidden neurons must be bounded [-1, 1]

    # sampling hidden neurons given visible neurons
    def propup_original(self, v, W, b):
      '''Compute the sigmoid activation for hidden units given visible units'''
      h = tf.nn.relu(tf.nn.conv2d(v, W, strides=[1, 2, 2, 1], padding='SAME') + b);
      h = tf.layers.batch_normalization(h, scale=True, training=True, 
                                        reuse=tf.AUTO_REUSE)
    ...
    def get_train_ops2(self, xShape, Delta, epsilon, batch_size, learning_rate, W, b, perturbFMx, perturbFM_h, bn_index):
        # compute h terms
        propup = self.propup(self.input + perturbFMx, W, b, bn_index) + perturbFM_h
        # reconstruct v terms
    ...
    # get differentially private pre-training energy function for convolutional RBM
    def get_train_ops2_original(self, xShape, Delta, epsilon, batch_size, learning_rate, W, b, perturbFMx, perturbFM_h):       
        # compute h terms
        propup = self.propup_original(self.input + perturbFMx, W, b) + perturbFM_h
        # reconstruct v terms
        rc_v = self.decode2(xShape, propup, W, b, activation=tf.nn.relu);
