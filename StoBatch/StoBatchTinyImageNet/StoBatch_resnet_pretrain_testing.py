...
"""
Testing Scaleable DP with RESNET18, TinyImageNet, large batch setting
"""
...
from pretrain_resnet_taylor_large_epoch import trainParams, resnetParams

import os, sys

ITER_STEP_TESTING = 2000 #[100, 500, 1000, 2000]
FGSM_EPS_LIST = [0.3, 0.4, 0.5]
...
def PDP_resnet_test(TIN_data, resnet_params, train_params, test_params, all_params):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    with tf.device(GPU_NAME[0]):
      y_logits_test = test_inference(x_sb + noise, FM_h, keep_prob, pre_define_vars, resnet_params, train_params)
    ...
    with tf.device(GPU_NAME[0]):
      if attack_switch['ifgsm']:
        ...
        attack_tensor_testing_dict['ifgsm'] = ifgsm_obj.generate(x=x_sb, eps=mu_alpha, eps_iter=mu_alpha/test_params.iter_step_testing, nb_iter=test_params.iter_step_testing, clip_min=-1.0, clip_max=1.0)
    ...
    with tf.device(GPU_NAME[0]):
      if attack_switch['mim']:
        ...
        attack_tensor_testing_dict['mim'] = mim_obj.generate(x=x_sb, eps=mu_alpha, eps_iter=mu_alpha/test_params.iter_step_testing, nb_iter=test_params.iter_step_testing, decay_factor=1.0, clip_min=-1.0, clip_max=1.0)
    ...
    with tf.device(GPU_NAME[0]):
      if attack_switch['madry']:
        ...
        attack_tensor_testing_dict['madry'] = madry_obj.generate(x=x_sb, eps=mu_alpha, eps_iter=mu_alpha/test_params.iter_step_testing, nb_iter=test_params.iter_step_testing, clip_min=-1.0, clip_max=1.0)
...
class testParams():
  def __init__(self, train_params):
    ...
    self.fgsm_eps_list = FGSM_EPS_LIST
    self.iter_step_testing = ITER_STEP_TESTING #[100, 500, 1000, 2000]
    self.attacks = ['ifgsm','mim','madry']
    self.num_samples = 1000
    self.log_file_path = '/results/pretrain_resnet_taylor_large_epoch_{}_{}_steps_eps{}.txt'.format(self.epoch_to_test, self.iter_step_testing, self.fgsm_eps_list)

def main(argv=None):  # pylint: disable=unused-argument
  # load params
  params_save_path = '/SDP_params.pik'
  ...
  PDP_resnet_test(TIN_data, resnet_params, train_params, test_params, all_params)
